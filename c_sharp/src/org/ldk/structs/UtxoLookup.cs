...
public class UtxoLookup : CommonBase {
	...
	~UtxoLookup() {
		if (ptr != 0) { Bindings.UtxoLookupFree(ptr); }
	}

	private class LDKUtxoLookupHolder { internal UtxoLookup held; }
	private class LDKUtxoLookupImpl : Bindings.LDKUtxoLookup {
		internal LDKUtxoLookupImpl(IUtxoLookup arg, LDKUtxoLookupHolder impl_holder) { this.arg = arg; this.impl_holder = impl_holder; }
		private IUtxoLookup arg;
		private LDKUtxoLookupHolder impl_holder;
		public long GetUtxo(long _chain_hash, long _short_channel_id) {
	...
	public UtxoResult get_utxo(byte[] chain_hash, long short_channel_id) {
		long ret = Bindings.UtxoLookupGetUtxo(this.ptr, InternalUtils.EncodeUint8Array(InternalUtils.CheckArrLen(chain_hash, 32)), short_channel_id);
		GC.KeepAlive(this);
