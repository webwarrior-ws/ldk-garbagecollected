...
public interface IOffersMessageHandler {
...
public class OffersMessageHandler : CommonBase {
	...
	~OffersMessageHandler() {
		if (ptr != 0) { Bindings.OffersMessageHandlerFree(ptr); }
	}
	...
	private class LDKOffersMessageHandlerImpl : Bindings.LDKOffersMessageHandler {
		internal LDKOffersMessageHandlerImpl(IOffersMessageHandler arg, LDKOffersMessageHandlerHolder impl_holder) { this.arg = arg; this.impl_holder = impl_holder; }
		private IOffersMessageHandler arg;
		private LDKOffersMessageHandlerHolder impl_holder;
		public long HandleMessage(long _message) {
		...
		public long ReleasePendingMessages() {
	...
	public static OffersMessageHandler new_impl(IOffersMessageHandler arg) {
		...
		long[] ptr_idx = Bindings.LDKOffersMessageHandlerNew(impl);
	...
	public Option_OffersMessageZ handle_message(org.ldk.structs.OffersMessage message) {
		long ret = Bindings.OffersMessageHandlerHandleMessage(this.ptr, message.ptr);
		GC.KeepAlive(this);
	...
	public ThreeTuple_OffersMessageDestinationBlindedPathZ[] release_pending_messages() {
		long ret = Bindings.OffersMessageHandlerReleasePendingMessages(this.ptr);
		GC.KeepAlive(this);
		if (ret >= 0 && ret <= 4096) { return null; }
