...
public class Option_EventZ : CommonBase {
	...
	~Option_EventZ() {
		if (ptr != 0) { Bindings.COptionEventZFree(ptr); }
	}

	internal static Option_EventZ constr_from_ptr(long ptr) {
		long raw_ty = Bindings.LDKCOptionEventZTyFromPtr(ptr);
		switch (raw_ty) {
	...
	public class Option_EventZ_Some : Option_EventZ {
		...
		internal Option_EventZ_Some(long ptr) : base(null, ptr) {
			long some = Bindings.LDKCOptionEventZSomeGetSome(ptr);
	...
	public static Option_EventZ some(org.ldk.structs.Event o) {
		long ret = Bindings.COptionEventZSome(o.ptr);
		GC.KeepAlive(o);
	...
	public static Option_EventZ none() {
		long ret = Bindings.COptionEventZNone();
		if (ret >= 0 && ret <= 4096) { return null; }
		org.ldk.structs.Option_EventZ ret_hu_conv = org.ldk.structs.Option_EventZ.constr_from_ptr(ret);
		if (ret_hu_conv != null) { ret_hu_conv.ptrs_to.AddLast(ret_hu_conv); };
		return ret_hu_conv;
	...
	internal long clone_ptr() {
		long ret = Bindings.COptionEventZClonePtr(this.ptr);
		GC.KeepAlive(this);
		return ret;
	}
	...
	public Option_EventZ clone() {
		long ret = Bindings.COptionEventZClone(this.ptr);
		GC.KeepAlive(this);
		if (ret >= 0 && ret <= 4096) { return null; }
		org.ldk.structs.Option_EventZ ret_hu_conv = org.ldk.structs.Option_EventZ.constr_from_ptr(ret);
