...
public class EventHandler : CommonBase {
	...
	~EventHandler() {
		if (ptr != 0) { Bindings.EventHandlerFree(ptr); }
	}

	private class LDKEventHandlerHolder { internal EventHandler held; }
	private class LDKEventHandlerImpl : Bindings.LDKEventHandler {
		internal LDKEventHandlerImpl(IEventHandler arg, LDKEventHandlerHolder impl_holder) { this.arg = arg; this.impl_holder = impl_holder; }
		private IEventHandler arg;
		private LDKEventHandlerHolder impl_holder;
		public void HandleEvent(long _event) {
	...
	public static EventHandler new_impl(IEventHandler arg) {
		...
		long[] ptr_idx = Bindings.LDKEventHandlerNew(impl);
	...
	public void handle_event(org.ldk.structs.Event _event) {
		Bindings.EventHandlerHandleEvent(this.ptr, _event.ptr);
		GC.KeepAlive(this);
		GC.KeepAlive(_event);
